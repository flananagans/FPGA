...
module top_level(
    ...
    input wire   copi_enco,          // (Controller-Out-Peripheral-In)
    output wire cipo_enco,          // (Controller-In-Peripheral-Out)
    input wire   dclk_enco,          // (Data Clock) - from controller
    input wire   cs_enco,             // (Chip Select) - from controller
    ...
    spi_peripheral #(.DATA_WIDTH(8)) spi_slave ( //teensy or psoc
        .clk(clk_100mhz),
        .rst(rst),
